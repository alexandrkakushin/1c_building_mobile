...
Функция ПолучитьВерсиюСервиса() Экспорт
	...
	Попытка
		...
		Соединение = Новый HTTPСоединение(СтруктураURI.Хост, СтруктураURI.Порт, Пользователь, Пароль, , 600);		
	Исключение
	...
	ОтветСервера = Соединение.Получить(
		Новый HTTPЗапрос(СтруктураURI.ПутьНаСервере, Заголовки));
